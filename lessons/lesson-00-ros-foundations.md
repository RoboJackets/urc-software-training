...
- `colcon build` produces `build/`, `install/`, and `log/` directories (git-ignored).
  These files are automatically generated by the compiler and you shouldn't have to
  touch them. Though sometimes you might have to delete files in 'build/' and 'install/' 
  if, for example, you delete a package and want to colcon build again. The corresponding 
  build and install files won't automatically be deleted.
